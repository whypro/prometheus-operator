...
func generateK8SSDConfig(namespaces []string, apiserverConfig *v1.APIServerConfig, basicAuthSecrets map[string]BasicAuthCredentials) yaml.MapItem {
	...
	if apiserverConfig != nil {
		...
		if apiserverConfig.TLSConfig != nil {
			tlsConfig := yaml.MapSlice{
				{Key: "insecure_skip_verify", Value: apiserverConfig.TLSConfig.InsecureSkipVerify},
			}
			if apiserverConfig.TLSConfig.CAFile != "" {
				tlsConfig = append(tlsConfig, yaml.MapItem{Key: "ca_file", Value: apiserverConfig.TLSConfig.CAFile})
			}
			if apiserverConfig.TLSConfig.CertFile != "" {
				tlsConfig = append(tlsConfig, yaml.MapItem{Key: "cert_file", Value: apiserverConfig.TLSConfig.CertFile})
			}
			if apiserverConfig.TLSConfig.KeyFile != "" {
				tlsConfig = append(tlsConfig, yaml.MapItem{Key: "key_file", Value: apiserverConfig.TLSConfig.KeyFile})
			}
			if apiserverConfig.TLSConfig.ServerName != "" {
				tlsConfig = append(tlsConfig, yaml.MapItem{Key: "server_name", Value: apiserverConfig.TLSConfig.ServerName})
			}
			k8sSDConfig = append(k8sSDConfig, yaml.MapItem{Key: "tls_config", Value: tlsConfig})
		}
	}
