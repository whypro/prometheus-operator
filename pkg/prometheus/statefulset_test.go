...
func TestMemoryRequestNotAdjustedWhenLimitLarger2Gi(t *testing.T) {
	...
	if requestString != "2Gi" {
		t.Fatalf("Resource request is expected to be 1Gi, instead found %v", resourceRequest)
	}
	if limitString != "3Gi" {
		t.Fatalf("Resource limit is expected to be 1Gi, instead found %v", resourceLimit)
	}
}
